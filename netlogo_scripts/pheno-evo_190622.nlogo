...
to make-environment
  ...
  ;; global variables determined by buttons:
...
to add-cells
  crt n [ ;; make cells
    ...
    set degrade-rate ( ;; determine phenotype (toxin degration capacity) by drawing from the 5-part genotypic distribution
      0.2 * (random-float item 0 sig-vals + (0.5 * item 0 mu-vals) +
      random-float item 1 sig-vals + (0.5 * item 1 mu-vals) +
      random-float item 2 sig-vals + (0.5 * item 2 mu-vals) +
      random-float item 3 sig-vals + (0.5 * item 3 mu-vals) +
      random-float item 4 sig-vals + (0.5 * item 4 mu-vals)))
    color-turtles
        ...
        if random-float 1 < growth-rate [ ;; draw from the probability of reproducing - only if yes, continue
          ...
          if random-float 1 < mutation-rate [ ; do a random draw to see whether it's time to mutate
            foreach [0 1 2 3 4][[x] -> ; loop through all mu-values and all sig-values. at each list index between 0 and 4...
              ...
              while [new-mu - (0.5 * new-sig) < 0] [ ; check whether the distribution goes negative. if it does, repeat the process until it doesn't.
                set new-mu old-mu + (random-float mu-step - 0.5 * mu-step)
                set new-sig old-sig + (random-float sig-step - 0.5 * sig-step)
              ]
              ;; beware: sometimes the following step makes the model freeze...
              while [new-mu + (0.5 * new-sig) > 1] [ ; check whether the distribution goes above 1. if it does, repeat the process until it doesn't.
                set new-mu old-mu + (random-float mu-step - 0.5 * mu-step)
                set new-sig old-sig + (random-float sig-step - 0.5 * sig-step)
              ]
          ...
          ask one-of growth-space [ ;; choose one of the empty patches to sprout
            sprout 1 [ ;; below are all the properties of the newly sprouted cell
              ...
              [set degrade-rate ( ;; if not, determine phenotype (toxin degration capacity) by drawing from the 5-part genotypic distribution
                0.2 * (random-float item 0 sig-vals + (0.5 * item 0 mu-vals) +
                  random-float item 1 sig-vals + (0.5 * item 1 mu-vals) +
                  random-float item 2 sig-vals + (0.5 * item 2 mu-vals) +
                  random-float item 3 sig-vals + (0.5 * item 3 mu-vals) +
