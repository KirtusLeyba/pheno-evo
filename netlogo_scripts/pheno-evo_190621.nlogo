...
globals [
  ...
  diff-rate ;; rate at which food and toxin diffuse from one cell to the next (per timestep)
  ...
  ; n ;; number of turtles ; determined by button
  ...
  ; alpha ;; exponent for relationship between growth rate and toxin degradation; determined by selector
]
...
to make-environment
  ...
  ;; set alpha 1 ;; unnecessary if there is a selector
...
to add-cells
  crt n [ ;; make 10 cells
    ...
    ;; color according to phenotype: low degradation ability = red, medium = yellow, high = blue
    set color palette:scale-gradient [[165 0 38][255 255 191][49 54 149]] degrade-rate 0 1
    set health 1
    set order-in-stack 1
    set barcode random n * 10
    set growth-rate basal-growth-rate * (1 - degrade-rate ^ alpha) ;; this deserves some serious thought
  ]
end
...
to poison ;; initiate toxin in one spot - this is mostly just to test diffusion
  ask patch random max-pxcor random max-pycor [ ;; choose random spot
    ...
    set pcolor scale-color gray (1 - log toxin 10) 0 200 ;; patch color is gray according to concentration
  ]
end
...
to dilute ;; dilute population - simulate transfer to fresh medium
  ; export-world (word "results " date-and-time ".csv") ;; note that this results in a rather large file - commenting out for now
...
to go
  ...
  ;    let my-toxin toxin
  ;    ask neighbors [
  ;      set toxin toxin + my-toxin * diff-rate / 8
  ;    ]
  ;    set toxin toxin - toxin * diff-rate
